...
class BufferModePipeline:
    ...
    def reset(self) -> None:
        ...
        self._stale_backup_intents_dropped = 0
        self._stale_backup_tokens_dropped = 0
        self._draining_before_device_eviction = False

    def is_idle(self) -> bool:
        """No queued or in-flight buffer-mode work."""
        return not (
    ...
    def drain_backups_before_device_eviction(self) -> None:
        """Admit metadata-only GPU PUT intents before device eviction.

        An admitted GPU-transient operation holds a tree lock, so normal device
        eviction can safely overlap its queued/running physical I/O. Only
        ``pending_write_queue`` entries are still unpinned. At the dependency
        boundary, drive scheduler acks until every such intent either acquires
        its source lock and enters the bounded backend queue or is found stale.
        """
        if (
            self._draining_before_device_eviction
            or not self.transient_buffer.requires_write_drain_before_device_eviction()
            or not self.pending_write_queue
        ):
            return

        self._draining_before_device_eviction = True
        started = time.perf_counter()
        initial_queue = len(self.pending_write_queue)
        initial_tokens = self.write_backlog_tokens_
        try:
            # First consume already-published completions and fill any capacity
            # that is available without waiting.
            self._cache.check_hicache_events()
            while self.pending_write_queue:
                self.transient_buffer.wait_for_progress()
                self._cache.check_hicache_events()
        finally:
            self._draining_before_device_eviction = False
        logger.info(
            "HiCache GPU-transient eviction barrier drained queue=%d "
            "backlog_tokens=%d duration_ms=%.3f stale_intents=%d "
            "stale_tokens=%d",
            initial_queue,
            initial_tokens,
            (time.perf_counter() - started) * 1000,
            self._stale_backup_intents_dropped,
            self._stale_backup_tokens_dropped,
        )
    ...
    def enqueue_backup_intent(self, node: UnifiedTreeNode) -> None:
        ...
        parent_covered = self._backup_parent_covered(node)
        oversize = self._backup_oversize(node, intent_tokens)
        if not parent_covered or oversize:
            self._log_backup_dropped(intent_tokens)
            return
    ...
    def _sweep_stale_backup_intents(self) -> None:
        ...
        for intent in self.pending_write_queue:
            if self._backup_intent_stale(intent):
                dropped_tokens = len(intent.hash_values) * page_size
                self.inflight_backup_node_ids.discard(intent.node_id)
                self.write_backlog_tokens_ -= dropped_tokens
                self._stale_backup_intents_dropped += 1
                self._stale_backup_tokens_dropped += dropped_tokens
                continue
    ...
    def flush_pending_writes(self) -> None:
        ...
        self.transient_buffer.drain_completions()
        if not self.pending_write_queue:
            return
    ...
    def _launch_backup_intent(self, intent: _UnifiedBackupIntent) -> bool:
        ...
        # Pin source ownership before an asynchronous backend can access it.
    ...
    def finish_storage_write_ack(self, operation_id: int) -> None:
        ...
        outright. The Host backend preserves legacy unconditional existence;
        GPU-transient adds it only after a rank-synchronized successful PUT.
        No-op for operations this pipeline does not own (for example, an ack
        for already-reset state)."""
        entry = self.ongoing_backup.pop(operation_id, None)
        if entry is None:
            return
        intent = entry.intent
        storage_succeeded = self.transient_buffer.storage_write_succeeded(entry.buffer)
        if storage_succeeded is not None:
            succeeded = torch.tensor(
                int(storage_succeeded), dtype=torch.int, device="cpu"
            )
            # MoonCake keys are PP-local (their suffix includes pp_rank), so
            # fold only the TP/CP participants that share this physical object.
            # _all_reduce() would broadcast PP0's unrelated outcome to every PP.
            self._cache._all_reduce_attn_groups(
                succeeded, torch.distributed.ReduceOp.MIN
            )
            storage_succeeded = bool(succeeded.item())
        if storage_succeeded is not False:
    ...
    def init_load_back(self, params: InitLoadBackParams) -> tuple[torch.Tensor, NodeId]:
        ...
        def _drop(_reason: str) -> tuple[torch.Tensor, NodeId]:
        ...
        if len(req.prefix_indices) != f.matched_len:
            ...
            # drop and recompute.
            return _drop("prefix_moved")

        if self.transient_buffer.restore_needs_device_allocation(f.buffer):
            # Host buffers allocate their device destination here. GPU-transient
            # GETs already own private destination pages, so a second capacity
            # check would evict unrelated cache entries for no reason.
        ...
        if restore.requires_ack:
            ...
            # The GPU destination is already filled and private. This insert
            # is the single publication point; ownership now belongs to the
            # tree and no L2 restore ack is required.
            self.transient_buffer.commit_restore(f.buffer)
            self.transient_buffer.release(f.buffer)
            cc.prefetch_tokens_occupied -= f.buffer.accounted_tokens
            if (
                cache.enable_storage_metrics
        ...
        if len(m.device_indices) < span_end:
            ...
            return unchanged
        # Another request may have inserted part of this span while the fetch
        # was staged. insert() frees those duplicate destination slots, so the
        # request must splice the authoritative tree mapping, not the original
        # backend allocation.
        return m.device_indices[f.matched_len : span_end], m.last_device_node
