"""Transient payload buffers used by HiCache buffer-only mode.

The buffer-mode scheduler pipeline owns ordering and cache visibility, while a
``TransientBufferBackend`` owns the memory used between the device cache and
storage.  The first implementation is Host-backed and deliberately preserves
the existing behavior.  Keeping the lease opaque to the pipeline is the seam
for a future fixed-size GPU ring implementation.
"""
