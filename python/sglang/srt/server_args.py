...
class ServerArgs:
    ...
    hicache_ratio: A[
        Optional[float],
        "The ratio of the size of host KV cache memory pool to the size of device pool. Defaults to 2.0 in cache mode, 1.2 in Host-backed buffer_only mode, or 0.2 for backup-only host-pool decode retraction. Must stay unset for gpu_transient storage I/O.",
        NS("memory"),
    ] = None
    hicache_size: A[
        int,
    ...
    def _handle_hicache_ratio_default(self):
        ...
        it against the retraction-backup backend (0.2 for backup-only host_pool,
        else 2.0).
        """
        if (
